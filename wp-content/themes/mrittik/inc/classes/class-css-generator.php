...
class Mrittik_CSS_Generator {
	...
	function __construct() {
		$this->opt_name = mrittik()->get_option_name();
	...
	function init() {
		...
		add_action( 'wp', array( $this, 'mrittik_generate_with_dev_mode' ) );
		add_action( "redux/options/{$this->opt_name}/saved", function () {
			$this->mrittik_generate_file_options();
		} );
	}

	function mrittik_generate_with_dev_mode() {
		if ( $this->dev_mode === true ) {
            $this->mrittik_generate_file_options();
			$this->mrittik_generate_file();
		}
	}

    function mrittik_generate_file_options() {
        ...
        $this->redux->filesystem->execute( 'put_contents', $_options, array(
            'content' => preg_replace( "/(?<=[^\r]|^)\n/", "\r\n", $this->mrittik_options_output() )
        ) );
    }

	function mrittik_generate_file() {
		$scss_dir = get_template_directory() . '/assets/scss/';
	...
	protected function mrittik_options_output() {
		$theme_colors                    = mrittik_configs('theme_colors');
        $links                           = mrittik_configs('link');
        $gradients                           = mrittik_configs('gradient');
		ob_start();

		printf('$mrittik_theme_colors_key:(%s);',$this->print_scss_opt_colors($theme_colors,'key'));
        printf('$mrittik_theme_colors_val:(%s);',$this->print_scss_opt_colors($theme_colors,'val'));
        // color rgb only
