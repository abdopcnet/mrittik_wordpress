<?php
if ( ! empty( $settings['btn_link_03']['url'] ) ) {
    $widget->add_render_attribute( 'btn_link_03', 'href', $settings['btn_link_03']['url'] );

    if ( $settings['btn_link_03']['is_external'] ) {
        $widget->add_render_attribute( 'btn_link_03', 'target', '_blank' );
    }
    if ( $settings['btn_link_03']['nofollow'] ) {
        $widget->add_render_attribute( 'btn_link_03', 'rel', 'nofollow' );
    }
}
$styleMappings = array(
...
?>
<div class="pxl-image-box pxl-image-box3 <?php echo esc_attr($settings['pxl_animate']); ?>" data-wow-delay="<?php echo esc_attr($settings['pxl_animate_delay']); ?>ms" data-wow-duration="<?php echo esc_attr($settings['pxl_animate_duration']); ?>s">
    <div class="pxl-item--inner">
        <?php if($settings['show_particle'] == 'true') : ?>
            <span class="image--reveal reveal-one wow fadeInDown" data-wow-duration="1s">
                <svg width="179" height="325" viewBox="0 0 179 325" fill="none" xmlns="http://www.w3.org/2000/svg" data-parallax='{"x": -20}'>
            ...
            <span class="image--reveal reveal-two wow fadeInUp" data-wow-duration="1s">
                <svg width="179" height="325" viewBox="0 0 179 325" fill="none" xmlns="http://www.w3.org/2000/svg" data-parallax='{"x": 20}'>
                    <path d="M283.138 337L-2 51.8625V50.6875L284.312 337H283.138Z" fill="white"/>
                    <path d="M259.329 337L-2 75.6709V74.4958L260.504 337H259.329Z" fill="white"/>
                    <path d="M235.521 337L-2 99.4792V98.3042L236.696 337H235.521Z" fill="white"/>
        ...
        <?php endif; ?>
        <?php if ( !empty($settings['image_front_01']['id']) && !empty($settings['image_front_02']['id']) ) : ?>
            <div class="item--front">
                <div class="canvas"></div>
                <div class="item--image">
                    <?php if ( ! empty( $settings['btn_link_03']['url'] ) ) { ?><a <?php pxl_print_html($widget->get_render_attribute_string( 'btn_link_03' )); ?>><?php } ?>
                        <img src="<?php echo esc_url($settings['image_front_01']['url'])?>" class="image-front attachment-full" alt="image front" data-sampler="texture0">
                        <img src="<?php echo esc_url($settings['image_front_02']['url'])?>" class="image-back attachment-full" alt="image back" data-sampler="texture1">
                        <img src="<?php echo get_template_directory_uri().'/assets/img/'.$imgds; ?>" class="map" alt="image displacements" data-sampler="map">
                    <?php if ( ! empty( $settings['btn_link_03']['url'] ) ) { ?></a><?php } ?>
                </div>
            </div>
        <?php endif; ?>
        <?php if ( !empty($settings['image_back_01']['id']) ) : ?>
            <div class="item--back">
                <?php if ( ! empty( $settings['btn_link_03']['url'] ) ) { ?><a <?php pxl_print_html($widget->get_render_attribute_string( 'btn_link_03' )); ?>><?php } ?>
                    <img src="<?php echo esc_url($settings['image_back_01']['url'])?>" class="image-front attachment-full" alt="image front" data-sampler="texture0">
                <?php if ( ! empty( $settings['btn_link_03']['url'] ) ) { ?></a><?php } ?>
            </div>
        <?php endif; ?>
    </div>
